...
test('companion apps read the same external graph through real Electron IPC without seeding or saving', { timeout: 90000 }, async t => {
  ...
  for (const name of ['remote-execution-studio', 'data-sources', 'kube-studio']) {
    const app = await electron.launch({ executablePath: process.env.ELECTRON_BIN, args: [path.resolve(__dirname, '../../../', name, 'main.js'), '--no-sandbox', '--disable-gpu'], env: { ...process.env, HOME: path.join(tmp, name), ROBOS_GRAPH_ROOT: path.dirname(ws.root) } });
    try {
