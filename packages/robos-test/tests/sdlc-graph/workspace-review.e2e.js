...
test('Explorer: prepare evidence brief, preview/refine/discard, save and reopen a real graph', { timeout: 90000 }, async t => {
  ...
  const startApp = () => electron.launch({ executablePath: process.env.ELECTRON_BIN, args: [path.resolve(__dirname, '../../../robos-graph/main.js'), '--no-sandbox', '--disable-gpu', '--disable-dev-shm-usage'], env: { ...process.env, ROBOS_GRAPH_ROOT: tmp, ROBOS_TEST: '1' } });
  let app;
  try {
    app = await startApp();
